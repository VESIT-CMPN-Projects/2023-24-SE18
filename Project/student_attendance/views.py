...
from users.models import student_profile,user_profile
...
@login_required(login_url="login")
@decorators.check_user_able_to_see_page(constants.Group.superadmin, constants.Group.teacher)
def attendance_view_class_wise_fn(request):
    ...
    return render(request, "student_attendance/manage_class_wise_student_attendance_view_for_heads.html", data)
...
def ajax_load_class_based_on_session_and_student(request):
    logged_student = request.user
    academic_session_id = request.GET.get("academic_session_id")
    
    class_data = class_details.objects.filter(
        Q(id__in=class_student.objects.filter(student=logged_student).values_list('stud_class', flat=True)) &
        Q(id__in=session_class.objects.filter(session=academic_session_id).values_list('class_det', flat=True))
    ).order_by("-created_dt")

    return render(
        request,
        "generic/class_dropdown_list_options.html",  # Adjust the template path accordingly
        {"classes": class_data},
    )

def ajax_load_students_from_class(request):
    print("funcc called")
    class_id = request.GET.get("class_id")
    print(class_id)
    ans=class_student.objects.filter(stud_class=class_id)
    print(ans)

    
    return render(
        request,
        "generic/student_dropdown_list_options.html",
        {"student_data":ans}
